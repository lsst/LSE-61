...
The following definition of requirement priorities is used in this document:

\begin{description}
  \item[\#1] ``This must be done to enter commissioning (a) or operations (b); no waivers will be granted if not met.''
  \begin{description}
  ...
  \end{description}
  \item[\#2:] ``Should be done to enter Ops; but waiver is likely to be granted if not met,'' i.e. we could enter operations w/o this fulfilled, for first 3 years.
  \item[\#3:] ``Overall capability/efficiency/ease of use/etc. may be reduced but science will not critically suffer if not done.''
  Could enter operations w/o this requirement fulfilled, and have the operations team decide whether they want to pursue it
\end{description}
...
\subsection{Publishing predicted telescope schedule}

\label{DMS-REQ-0353}
\textbf{ID:} DMS-REQ-0353 (Priority: 1b)

\textbf{Specification:}
A service shall be provided to publish to the community the next visit location and the predicted telescope schedule provided by the OCS. This service shall consist of both a web page for human inspection and a web API to allow automated tools to respond promptly.
...
\subsubsection{EFD Query Performance}

\label{DMS-REQ-0354}
\textbf{ID:} DMS-REQ-0354

\textbf{Specification:}
The queryable EFD shall support at least 5 simultaneous queries, assuming each query lasts no more than 10 seconds.

\subsubsection{L1 Database Query Performance}

\label{DMS-REQ-0355}
\textbf{ID:} DMS-REQ-0355

\textbf{Specification:}
The live L1 database shall support at least 20 simultaneous queries, assuming each query lasts no more than 10 seconds.

\subsubsection{Real time query performance}

\label{DMS-REQ-0356}
\textbf{ID:} DMS-REQ-0356

\textbf{Specification:}
Low volume queries shall respond within 10 seconds under a load of approximately 100 simultaneous queries.
...
\subsubsection{Fast turnaround for high-volume queries}

\label{DMS-REQ-0357}
\textbf{ID:} DMS-REQ-0357

\textbf{Specification:}
High-volume queries – queries that involve full-sky scans – shall be answered in 1 hour, while more complex full-sky spatial and temporal correlations shall be answered in less than 12 hours. The system shall support 50 simultaneous high-volume queries to be running at any given time.

\subsection{Regenerating Data Products from Previous Data Releases}

\label{DMS-REQ-0336}
\textbf{ID:} DMS-REQ-0336 (Priority: 2)
