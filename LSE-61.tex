...
\paragraph{Produce All-Sky HiPS Map}\hfill  % Force subsequent text onto new line
...
\paragraph{HiPS Service}\hfill  % Force subsequent text onto new line
...
\paragraph{HiPS Visualization}\hfill  % Force subsequent text onto new line
...
\textbf{Discussion:}
Further details will be provided in the LSST Science Platform Requirements, \citeds{LDM-554}.
...
\paragraph{Produce MOC Maps}\hfill  % Force subsequent text onto new line
...
\paragraph{Export MOCs As FITS}\hfill  % Force subsequent text onto new line
...
\paragraph{MOC Visualization}\hfill  % Force subsequent text onto new line
...
\paragraph{Alert Stream Distribution}\hfill  % Force subsequent text onto new line

\label{DMS-REQ-0391}
\textbf{ID:} DMS-REQ-0391

\textbf{Specification:}
\textbf{Specification}: LSST \textit{shall} be capable of supporting the transmission of at least \textbf{NumStreams} full alert streams out of the alert distribution system within \textbf{OTT1}.

\textbf{Discussion:}
\textbf{Discussion}: This requirement establishes the minimum capacity needed for timely transmission of alerts to community brokers.

\\
Derived from Requirements: OSS-REQ-0127, OSS-REQ-0184
...
\paragraph{Alert Delay and Failure Tolerances}\hfill  % Force subsequent text onto new line

\label{DMS-REQ-0392}
\textbf{ID:} DMS-REQ-0392

\textbf{Specification:}
\textbf{Specification}: The system \textit{shall} reliably produce alerts for standard science visits read out in the camera [and specified to be analyzed by Data Management] such that no more than \textbf{sciVisitAlertDelay} % of visits will fail to have at least \textbf{OTR1} % of its alerts distributed via the LSST alert distribution system within \textbf{OTT1}, and no more than \textbf{sciVisitAlertFailure} % of visits will fail to generate and distribute alerts (integrated over all stages of data handling)

\textbf{Discussion:}
\textbf{Discussion}: As with \hyperref[DMS-REQ-0004]{DMS-REQ-0004}, this specification applies to visits which should have resulted in fewer than \textbf{nAlertVisitPeak}.

\\
Derived from Requirements: OSS-REQ-0112

\begin{parameters}
Maximum fraction of science visits with less than OTR1 % of the alerts distributed within OTT1
\\
\\
\\
\\
...
\paragraph{Performance Requirements for Transient Alert Distribution}\hfill  % Force subsequent text onto new line

\label{DMS-REQ-0393}
\textbf{ID:} DMS-REQ-0393

\textbf{Specification:}
\textbf{Specification}: The system \textit{shall} be able to identify and distribute an average of at least \textbf{nAlertVisitAvg} alerts per standard visit during a given night, and at least \textbf{nAlertVisitPeak} for a single standard visit.

Performance \textit{shall} degrade gracefully beyond that limit (average of at least \textbf{nAlertVisitAvg} alerts per standard visit during a given night).

\textbf{Discussion:}
\textbf{Discussion}: The term 'degrade gracefully' means that visits with an excess of difference-image sources should not cause any DMS downtime; i.e., the system does not crash and is able to distribute alerts from that visit, potentially with greater latency.
...
\paragraph{Provide Re-Run Tools}\hfill  % Force subsequent text onto new line
...
\paragraph{Re-Runs on Similar Systems}\hfill  % Force subsequent text onto new line
...
\paragraph{Re-Runs on Other Systems}\hfill  % Force subsequent text onto new line
...
\textbf{Specification:}
The DMS shall include software to enable the calculation of the elipticity correlations metrics defined in OSS-REQ-0390.

\textbf{Discussion:}
The relevant metrics are listed in the table below. The values in the tables are the target values for LSST but are not verified as part of this requirement.

\begin{parameters}
Median residual PSF ellipticity correlations averaged over an arbitrary field of view for separations less than 5 arcmin shall be no greater than\textbf{ TE2.}
&
1.0e-7
&
\unitname{%
arcminuteSeparationCorrelation
}
&
\paramname{%
TE2
...
The outlier limit on the PSF ellipticity correlation residuals on 5 arcminute scales shall be no more than \textbf{TE4.}
&
2.0e-7
&
\unitname{%
arcminuteOutlierLimit
}
&
\paramname{%
TE4
} \\\hline
Median residual PSF ellipticity correlations averaged over an arbitrary field of view for separations less than 1 arcmin shall be no greater than \textbf{TE1.}
&
2.0e-5
&
\unitname{%
...
The outlier limit on the PSF ellipticity correlation residuals on 1 arcminute scales shall be no more than \textbf{TE3.}
&
4.0e-5
&
\unitname{%
...
These requirements were originally approved in narrative text in \citeds{LDM-135}.
